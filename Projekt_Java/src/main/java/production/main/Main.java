...
import production.exceptions.checkedExceptions.DateAfterToday;
import production.exceptions.checkedExceptions.DateBeforeToday;
import production.enums.City;
import production.enums.TypeOfTransportation;
import production.exceptions.uncheckedExceptions.AccommodationNameAlreadyExistsException;
...
import production.builderPatterns.AddressBuilder;
import production.records.Discount;
import production.exceptions.uncheckedExceptions.ValidNumberException;
...
public class Main
{
    ...
    private static void enterPassenger(int i,Scanner inputScanner,Set<Passenger> passengers)
    {
        ...
        {
            error = false;
            System.out.println("Unesite korisničko ime " + (i + 1) + ". putnika");
            passengerUsername = inputScanner.nextLine();
            for (User user:userList)
            {
                if (user.getUsername().equals(passengerUsername) && user.getRole().equals(Role.MEMBER))
                {
                    error=true;
                    System.out.println("Molimo Vas da unesete korisničko ime koje se ne koristi.");
                    logger.error("Pogreška! Korisnik je upisao korisničko ime koje se koristi");
                }
            }
        }
        while(error);
        System.out.println("Unesite lozinku "+(i+1)+". putnika");
        String passengerPassword=inputScanner.nextLine();
        passengers.add(new Passenger(passengerId,passengerName,passengerSurname,passengerEmail,passengerPhoneNumber,passengerDateOfBirth,passengerUsername,passengerPassword));
        Long userID=FileUtils.getNextUserID();
        userList.add(new User(userID,passengerUsername,passengerPassword,Role.MEMBER));
    ...
    private static void outputUsers(List<User> userList)
    {
        for (User user: userList)
        {
            System.out.println("ID: "+user.getId()+", Username: "+user.getUsername()+", Password: "+user.getPassword()+", Role: "+user.getRole());
        }
    }
}